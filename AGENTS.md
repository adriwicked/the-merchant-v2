...
```
src/
  config.ts        — Grid constants, color palette, shared helpers (tweakColor, cellPosition)
  ...
  mine-view.ts     — Mine rendering (tile colors, player, bombs, nuggets)
...
### Color Manipulation
All colors are `0xRRGGBB` numbers manipulated via bit shifting. Helper in `config.ts`:
...
### Animations
Water animation uses a dedicated `Phaser.GameObjects.Graphics` layer, base colors stored in state, re-tweaked every 2 seconds via `this.time.addEvent`.
...
- MineTile enum: BLACK(0), WALL(1), FLOOR(2)
- Very dark colors (WALL `0x382a1a`, FLOOR `0x3d332a`)
- Bombs: space to plant, 2s fuse, irregular explosion radius 3-5, 15% gold nugget spawn per converted tile
- Post-explosion: wall regeneration pass (BLACK tiles adjacent to FLOOR become WALL)
